...
def main():
    ...
    feat = np.load(root + 'features.npy')
    if args.graphlets == True:
        config = [
            ('GraphConv', [feat.shape[1], args.hidden_dim]),
    ...
    with open(root + 'nodes_idx.pkl', 'rb') as f:
        nodes_idx = pickle.load(f) 

    root = root + 'fold' + str(args.fold_n) + '/'
    # batchsz here means total episode number
    db_train = Subgraphs(root, 'train', total_subgraph, info, center_node, nodes_idx, n_way=args.n_way, k_shot=args.k_spt, k_query=args.k_qry, batchsz=1000)
    db_val = Subgraphs(root, 'val', total_subgraph, info, center_node, nodes_idx, n_way=args.n_way, k_shot=args.k_spt,k_query=args.k_qry, batchsz=100)
    db_test = Subgraphs(root, 'test', total_subgraph, info, center_node, nodes_idx, n_way=args.n_way, k_shot=args.k_spt,k_query=args.k_qry, batchsz=100)
    ...
    if args.no_finetune==True:
        for epoch in range(args.epoch):
            ...
            for step, (x_spt, y_spt, x_qry, y_qry, c_spt, c_qry, n_spt, n_qry) in enumerate(db):
                ...
                accs, _, _ = maml(x_spt, y_spt, x_qry, y_qry, c_spt, c_qry, n_spt, n_qry, feat, graphlets)
                if step % 1 == 0:
                ...
                if step % 500 == 0:  # evaluation
                    ...
                    for x_spt, y_spt, x_qry, y_qry, c_spt, c_qry, n_spt, n_qry in db_v:
                        #x_spt, y_spt, x_qry, y_qry = x_spt.to(device), y_spt.to(device), \
                        #                             x_qry.to(device), y_qry.to(device)

                        accs = maml.finetunning(x_spt, y_spt, x_qry, y_qry, c_spt, c_qry, n_spt, n_qry, feat, graphlets)
    ...
    for x_spt, y_spt, x_qry, y_qry, c_spt, c_qry, n_spt, n_qry in db_t:
        accs = maml.finetunning(x_spt, y_spt, x_qry, y_qry, c_spt, c_qry, n_spt, n_qry, feat, graphlets)
        accs_all_test.append(accs)

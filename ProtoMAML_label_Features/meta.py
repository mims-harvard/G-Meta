...
def proto_loss_spt_1(logits, y_t, n_support):
    n_support = 1
...
class Meta(nn.Module):
    ...
    def forward(self, x_spt, y_spt, x_qry, y_qry, c_spt, c_qry, n_spt, n_qry, feat, graphlets):
        """
        b: number of tasks
        ...
        for i in range(task_num):
            feat_spt = torch.Tensor(np.vstack(([feat[np.array(x)] for x in n_spt[i]]))).to(device)
            feat_qry = torch.Tensor(np.vstack(([feat[np.array(x)] for x in n_qry[i]]))).to(device)
        ...
        return accs, [i.detach().numpy() for i in losses_q], [i.detach().numpy() for i in losses_s]


    def finetunning(self, x_spt, y_spt, x_qry, y_qry, c_spt, c_qry, n_spt, n_qry, feat, graphlets):
        """
        ...
        feat_spt = torch.Tensor(np.vstack(([feat[np.array(x)] for x in n_spt]))).to(device)
        feat_qry = torch.Tensor(np.vstack(([feat[np.array(x)] for x in n_qry]))).to(device)

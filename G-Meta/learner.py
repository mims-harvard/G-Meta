...
class Classifier(nn.Module):
    def __init__(self, config):
        ...
        self.graph_conv = []
        self.graph_conv_graphlets = []
        self.config = config
        self.graphlets_mode = False
        ...
        for i, (name, param) in enumerate(self.config):
            ...
            if name is 'Graphlet':
                ...
                self.vars.append(w)
                self.vars.append(nn.Parameter(torch.zeros(param[1])))
                self.graph_conv_graphlets.append(GraphConv(param[0], param[1], activation = F.relu))

                w = nn.Parameter(torch.Tensor(param[1], param[1]))
                init.xavier_uniform_(w)
                self.vars.append(w)
                self.vars.append(nn.Parameter(torch.zeros(param[1])))
                self.graph_conv_graphlets.append(GraphConv(param[1], param[1], activation = F.relu))



    def forward(self, g, to_fetch, graphlets, features, vars = None):
